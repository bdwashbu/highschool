...
bool textureclass::LoadTGA(int number,char *filename, bool linear)// Loads A TGA File Into Memory
{   
	texID = number;
...
void textureclass::BuildFont()
{
	...
	for (loop=0; loop<95; loop++)// Loop Through All 256 Lists
	{
		...
		glBegin(GL_QUADS);// Use A Quad For Each Character
			glTexCoord2f(cx+0.001,1.0f-cy-0.120f);// Texture Coord (Bottom Left)
			glVertex2d(0,0);// Vertex Coord (Bottom Left)
			glTexCoord2f(cx+0.0620f,1.0f-cy-0.120f);// Texture Coord (Bottom Right)
			glVertex2i(16,0);// Vertex Coord (Bottom Right)
			glTexCoord2f(cx+0.0620f,0.980f-cy);// Texture Coord (Top Right)
			glVertex2i(16,13);// Vertex Coord (Top Right)
			glTexCoord2f(cx+0.001,0.980f-cy);// Texture Coord (Top Left)
...
void textureclass::glPrint(float size, GLfloat x, GLfloat y, int set, const char *string, ...)// Where The Printing Happens
{
    ...
    if (!fp) {
		...
		for (int x = 0; x < w; x++, colorpointer+=4)
		{
			if (colorpointer[0] > 0.2f || colorpointer[1] > 0.2f || colorpointer[2] > 0.2f)
				colorpointer[3] = char(((255.0f - sqrtf((y - float(h/2))*(y-float(h/2))+(x-float(w/2))*(x-float(w/2))) / (w/2) * 255.0f)));
			else
				colorpointer[3] = 0;
		}
		}
