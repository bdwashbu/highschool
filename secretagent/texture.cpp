...
bool tga::LoadTGA(int number,char *filename)// Loads A TGA File Into Memory
{   
	texID = number;
...
void tga::glPrint(float size, GLint x, GLint y, int set, const char *string, ...)// Where The Printing Happens
{
...
bool tga::SCREENSHOT_WriteTGA(const char *fname,int w, int h)
{
    ...
    if (!fp) {
		...
		for (int x = 0; x < w; x++, colorpointer+=4)
		{
			if (colorpointer[0] > 0.2f || colorpointer[1] > 0.2f || colorpointer[2] > 0.2f)
				colorpointer[3] = ((255.0f - sqrtf((y - h/2)*(y-h/2)+(x-w/2)*(x-w/2)) / (w/2) * 255.0f));
			else
				colorpointer[3] = 0;
		}
		}
