...
{	
	...
	float timelimit, phaseshift;
	begintime = timeGetTime();
	glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
	
	phaseshift++;
	time+=scalar;
	timelimit = 0.05f+0.04*sin(phaseshift/30*(3.14f/180.0f));
	bool setline = false;
	for (int i = 0; i < 200; i++) {
		line[i].DrawLine(scalar);
		if (setline == false && time > timelimit) {
			if (line[i].SetLine(rand()%640, rand()%480, rand()%640, rand()%480)) {
				time = 0.0f;
				setline = true;
			}
		}
